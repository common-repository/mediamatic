...
class Mediamatic_Category{
	...
	public function __construct()
	{
		...
		add_filter( 'ajax_query_attachments_args', array( $this, 'mediamaticQueryAttachmentsArgs' ) ); 
		add_action( 'wp_ajax_save-attachment-compat', array( $this, 'mediamaticSaveAttachmentCompat' ), 0 );
	}
	
	public function mediamaticAddAttachmentCategory( $postID ) 
	{
        $mediamaticFolder 	= isset($_REQUEST["ccFolder"]) ? sanitize_text_field($_REQUEST["ccFolder"]) : null;
        if(is_null($mediamaticFolder)){
            $mediamaticFolder = isset($_REQUEST["cc_mediamatic_folder"]) ? sanitize_text_field($_REQUEST["cc_mediamatic_folder"]) : null;
        }
        if($mediamaticFolder !== null){
            $mediamaticFolder = (int)$mediamaticFolder;
            if($mediamaticFolder > 0){
                wp_set_object_terms($postID, $mediamaticFolder, MEDIAMATIC_FOLDER, false);
            }
        }
    }


    public function mediamaticSetAttachmentCategory($postID)
	{
	...
	public function mediamaticQueryAttachmentsArgs( $query = array() )
	{
        $taxquery 			= array();
        ...
        foreach($taxonomies as $taxonomy){
            if(isset($query[$taxonomy]) && is_numeric($query[$taxonomy])){
                if($query[ $taxonomy ] > 0){
                    ...
                    $allTermsIDs = self::mediamaticGetTermsValues('ids');
                    array_push( $query[ 'tax_query' ], array(
                        'taxonomy' => $taxonomy,
                        'field'    => 'id',
                        'terms'    => $allTermsIDs,
